...
contract CustomTokenUpgradable is
    Initializable,
    UUPSUpgradeable,
    PausableUpgradeable,
    OwnableUpgradeable
{
    ...
    event UpdatePercent(
        uint256 _newDisPercent,
        uint256 _newLiqPercent,
    ...
    function updateAccumulateBalance(address _holder) private {
        if (accAmount[_holder].lastUpdateTime == 0) {
            if (_getRewardStartTime == 0) {
                uint256 interval = (block.timestamp - _lastRewardStartTime) /
                    _averageInterval;
                accAmount[_holder].lastUpdateTime =
                    _lastRewardStartTime +
                    (interval * _averageInterval);
            } else {
                if (block.timestamp < _getRewardStartTime) {
                    uint256 interval = (block.timestamp -
                        _lastRewardStartTime) / _averageInterval;
                    accAmount[_holder].lastUpdateTime =
                        _lastRewardStartTime +
                        (interval * _averageInterval);
                } else {
                    uint256 interval = (block.timestamp - _getRewardStartTime) /
                        _averageInterval;
                    accAmount[_holder].lastUpdateTime =
                        _getRewardStartTime +
                        (interval * _averageInterval);
            ...
            if (accAmount[_holder].lastUpdateTime < _lastRewardStartTime) {
                ...
                uint256 interval = (block.timestamp -
                    accAmount[_holder].lastUpdateTime) / _averageInterval;
                if (interval >= 1) {
                    uint256 accumulateAmount = balanceOf[_holder] * interval;
                    accAmount[_holder].lastUpdateTime += (interval *
                        _averageInterval);
                    accAmount[_holder].amount =
                        accAmount[_holder].amount +
                        accumulateAmount;
                }
            }
            ...
            if (accAmount[_holder].lastUpdateTime < _lastRewardStartTime) {
                uint256 interval = (_getRewardStartTime -
                    _lastRewardStartTime) / _averageInterval;
                uint256 lastmonthAccAmount = balanceOf[_holder] * interval;
                accAmount[_holder].amount = 0;
                accAmount[_holder].accReward =
                    (lastmonthAccAmount *
                        _monthlyDistributePr *
                        _percentageDistribute *
                        100) /
                    _totalSupply /
                    _numOfDaysPerMth /
                    10000;
                ...
                accAmount[_holder].lastUpdateTime >= _lastRewardStartTime &&
                accAmount[_holder].lastUpdateTime < _getRewardStartTime
            ) {
                uint256 interval = (_getRewardStartTime -
                    accAmount[_holder].lastUpdateTime) / _averageInterval;
    ...
    function updateAccumulateBalanceTransaction(
        address _from,
        address _to
    ) private {
    ...
    function claimDistributionPurse()
        external
        whenNotPaused
    ...
    function activateClaimMonthly(
        uint256 _rewardStartTime,
        uint256 _rewardEndTime,
        uint256 _monthlyDisPr,
        uint256 _numOfDays,
        uint256 _percentage
    ) external onlyAdmin {
        ...
        require(
            _monthlyDisPr <= balanceOf[address(this)],
            "More than pool balance"
        );
    ...
    function _partialBurn(
        ...
        uint256 liqAmount;
        uint256 disAmount;
        (burnAmount, liqAmount, disAmount) = _calculateDeductAmount(_amount);
    ...
    function _calculateDeductAmount(
        uint256 _amount
    ...
    function updatePercent(
        ...
        require(
            _newDisPercent + _newLiqPercent + _newBurnPercent <= 10000,
            "Total more than 10000"
        );
    ...
    function initialize(
        ...
        require(
            _disPercent + _liqPercent + _burnPercent <= 10000,
            "Total more than 10000"
        );
